...
class LeNet(nn.Module):
    """
    在传统机器学习的情况下，对样本作正则化可以提高收敛的速度。
    深度学习不对样本做正则化的情况下，一般在多层神经网络的传递下，越靠近输出层梯度越大，
    越靠近数据层梯度越小。靠近输入层还没拟合好，靠近输出层的部分已经拟合好了。
    在这种情况下网络深度很难做到很深。
    batch normalization做完归一化后，可以解决上面的问题，同时有论文提出，BN在一定程度上
    加入了噪音，提高了模型的稳健性。所以理论上没必要根dropout混用，一般不会有太多提升。
    BN本质上是一种线性变化。
    """
