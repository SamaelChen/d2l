...
predict(net, test_iter)
# %%
# ResNet是将函数分成简单的先行项和非线性项，类似泰勒展开，如果将函数分解成更多的部分，就是DenseNet。
# 与ResNet不同的是DenseNet是将输出concat起来的，而不是加和。


def conv_block(input_channels, num_channels):
    blks = []
    blks.append(nn.BatchNorm2d(input_channels))
    blks.append(nn.ReLU())
    blks.append(nn.Conv2d(input_channels, num_channels,
                kernel_size=3, padding=1))
...
class DenseBlock(nn.Module):
    """
    通道数增长，卷积块的通道数控制了输出通道数的增长，也叫做增长率
    """
    ...
    def __init__(self, num_convs, input_channels, num_channels):
        ...
        for i in range(num_convs):
            layers.append(conv_block(num_channels*i +
...
def transition_block(input_channels, num_channels):
    """
    通过卷积层减少通道数，再通过pooling进一步减少模型复杂度
    """
    layers = []
...
def densenet(input_channels, num_labels, num_dense_blks, num_channels=64, growth_rate=32):
    """
    DenseNet的第一部分与ResNet一样。之后稠密网络使用4个稠密块。
    """
    b1 = nn.Sequential(
        nn.Conv2d(input_channels, 64, kernel_size=7, stride=2, padding=1),
        nn.BatchNorm2d(64),
        nn.ReLU(),
        nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
    )
    blks = []
    for i, num_convs in enumerate(num_dense_blks):
        blks.append(DenseBlock(num_convs, num_channels, growth_rate))
        num_channels += num_convs * growth_rate
        if i != len(num_dense_blks) - 1:
    ...
    net = nn.Sequential(
        ...
        *blks,
        ...
        nn.Flatten(),
        nn.Linear(num_channels, num_labels)
    )
    return net


# %%
net = densenet(3, 9, [3, 4, 6, 3])
net = net.cuda()
summary(net, input_size=((3, 224, 224)))
optimizer = torch.optim.SGD(net.parameters(), lr=0.01)
epochs = 20
