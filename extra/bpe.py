...
def tokenize_word(string, sorted_tokens, unknown_token='</u>'):
    ...
    for i in range(len(sorted_tokens)):
        ...
        matched_positions = [(m.start(0), m.end(0))
                             for m in re.finditer(token_reg, string)]
        if len(matched_positions) == 0:
            continue
        substring_end_positions = [matched_position[0]
        ...
        string_tokens += tokenize_word(string=remaining_substring,
...
sorted_tokens_tuple = sorted(tokens_frequencies.items(), key=lambda item: (
    measure_token_length(item[0]), item[1]), reverse=True)
